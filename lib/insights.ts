...
export function selectInsights(answers: Answers, band: 'hot' | 'warm' | 'cold'): string[] {
  ...
  if (band === 'hot') {
    if (answers.q1 === 'q1_c' || answers.q1 === 'q1_d')
      bullets.push("The real status of your work lives across spreadsheets, notes, and people's heads. Consolidating that into one fitted system is where most of your recoverable time is.")
    ...
    if (answers.q3 === 'q3_c' || answers.q3 === 'q3_d')
      bullets.push('The same information gets re-entered in four or more places as a job moves through. Every one is a chance for an error and a few minutes lost, multiplied across every job.')
    const h = hoursBullet(answers)
    if (h) bullets.push(h)
    else if (answers.q4 === 'q4_c' || answers.q4 === 'q4_d')
      bullets.push('Rework is a weekly-or-worse occurrence for you. A system that catches the miss at the source removes most of it.')
  ...
  if (band === 'warm') {
    if (answers.q1 === 'q1_c' || answers.q1 === 'q1_d')
      bullets.push('A lot of your real operating detail lives outside any single system. That scattering is the core of what custom software fixes, and your score reflects it.')
    if (answers.q2 === 'q2_c' || answers.q2 === 'q2_d')
  ...
  if (answers.q8 === 'q8_c')
    bullets.push("Budget isn't in place yet, and custom software only makes sense once the ROI case is clear. That's the right thing to sort first.")
  if (answers.q9 === 'q9_c')
  ...
  bullets.push("This isn't a 'never,' it's a 'not yet.' Many businesses come back when they hit the next growth inflection and the conditions line up.")
  return bullets.slice(0, 4)
}
